...
# 读取女ASMR数据 ---------------------------------------------------------------
world <- map_data('world')
setwd("C:\\Users\\dell\\Desktop\\GBD数据\\地图\\3")

data1<- fread("筛选出204个国家2021年ASMR(女).csv",header = T)
...
map1 <- data1 %>%
  mutate(val2 = cut(val, breaks = c(0,0.13,0.30,0.60,0.90,1.20,100000000),
                    labels = c("<0.13","0.13~0.30","0.30~0.60","0.60~0.90","0.90~1.20",
                               ">1.20"),  
...
ggsave("ASMR女.JPEG",width = 12, height = 7,dpi = 1000) #使用ggsave()函数保存，图片质量好
...
data1<- fread("筛选出204个国家2021年ASDR(女).csv",header = T)
...
map1 <- data1 %>%
  mutate(val2 = cut(val, breaks = c(0,3.30,7.75,15.50,22.20,30.00,100000000),
                    labels = c("<3.30","3.30~7.75","7.75~15.50","15.50~22.20","22.20~30.00",
                               ">30.00"),  
                    include.lowest = T,right = T)) #建立新列，连续变量转换成分类变量，包含最小值所在区间且包括左端点而不包括右端点
...
ggsave("ASDR女.JPEG",width = 12, height = 7,dpi = 1000) #使用ggsave()函数保存，图片质量好
...
data1<- fread("204个国家肺癌的二手烟死亡标准化率EAPC（女）.csv",header = T)
...
map1 <- data1 %>%
  mutate(val2 = cut(EAPC, breaks = c(-6,-1.55,-0.40,0,0.70,1.70,100000000),
                    labels = c("<-1.55","-1.55~-0.40","-0.40~0.00","0.00~0.70","0.70~1.70",
                               ">1.70"),  
                    include.lowest = T,right = T)) #建立新列，连续变量转换成分类变量，包含最小值所在区间且包括左端点而不包括右端点
...
ggsave("EAPC in ASMR女.JPEG",width = 12, height = 7,dpi = 1000) #使用ggsave()函数保存，图片质量好
...
data1<- fread("204个国家肺癌的二手烟DALYS标准化率EAPC(女）.csv",header = T)
...
map1 <- data1 %>%
  mutate(val2 = cut(EAPC, breaks = c(-6,-1.60,-0.60,0,0.60,1.70,100000000),
                    labels = c("<-1.60","-1.60~-0.60","-0.60~0.00","0.00~0.60","0.60~1.70",
                               ">1.70"),  
                    include.lowest = T,right = T)) #建立新列，连续变量转换成分类变量，包含最小值所在区间且包括左端点而不包括右端点
...
ggsave("EAPC in ASDR女.JPEG",width = 12, height = 7,dpi = 1000) #使用ggsave()函数保存，图片质量好
...
# 读取图片  
fig1<- image_read("EAPC in ASMR男.JPEG")%>% image_ggplot()  
print(fig1)
fig2<- image_read("EAPC in ASDR男.JPEG")%>% image_ggplot()  
print(fig2)
fig3<- image_read("EAPC in ASMR女.JPEG")%>% image_ggplot()  
print(fig3)
fig4<- image_read("EAPC in ASDR女.JPEG")%>% image_ggplot()  
print(fig4)
...
ggsave("EAPC组合图片男女（AB为男，CD为女）.JPEG",width = 12, height = 7,dpi = 1000) #使用ggsave()函数保存，图片质量好
