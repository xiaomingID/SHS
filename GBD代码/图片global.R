...
world <- map_data('world')
#
setwd("C:\\Users\\dell\\Desktop\\GBD数据\\地图\\1")

data1<- fread("筛选出204个国家2021年ASMR(总).csv",header = T)
...
map1 <- data1 %>%
  mutate(val2 = cut(val, breaks = c(0,0.18,0.55,0.95,1.50,1.85,100000000),
                    labels = c("<0.18","0.18~0.55","0.55~0.95","0.95~1.50","1.50~1.85",
                              ">1.85"),  
...
ggsave("ASMR总.JPEG",width = 12, height = 7,dpi = 1000) #使用ggsave()函数保存，图片质量好
...
map1 <- data1 %>%
  mutate(val2 = cut(val, breaks = c(0,4.70,13.00,24.00,38.00,50.00,100000000),
                    labels = c("<4.70","4.70~13.00","13.00~24.00","24.00~38.00","38.00~50.00",
                               ">50.00"),  
                    include.lowest = T,right = T)) #建立新列，连续变量转换成分类变量，包含最小值所在区间且包括左端点而不包括右端点
...
ggsave("ASDR总.JPEG",width = 12, height = 7,dpi = 1000) #使用ggsave()函数保存，图片质量好
...
data1<- fread("204个国家肺癌的二手烟死亡标准化率EAPC（总）.csv",header = T)
...
map1 <- data1 %>%
  mutate(val2 = cut(EAPC, breaks = c(-5,-2.20,-1.08,0,0.50,1.00,100000000),
                    labels = c("<-2.20","-2.20~-1.08","-1.08~0.00","0.00~0.50","0.50~1.00",
                               ">1.00"),  
                    include.lowest = T,right = T)) #建立新列，连续变量转换成分类变量，包含最小值所在区间且包括左端点而不包括右端点
...
ggsave("EAPC in ASMR.JPEG",width = 12, height = 7,dpi = 1000) #使用ggsave()函数保存，图片质量好
...
data1<- fread("204个国家肺癌的二手烟DALYS标准化率EAPC（总）.csv",header = T)
...
map1 <- data1 %>%
  mutate(val2 = cut(EAPC, breaks = c(-5,-2.30,-1.18,0,0.50,1.00,100000000),
                    labels = c("<-2.30","-2.30~-1.18","-1.18~0.00","0.00~0.50","0.50~1.00",
                               ">1.0"),  
                    include.lowest = T,right = T)) #建立新列，连续变量转换成分类变量，包含最小值所在区间且包括左端点而不包括右端点
...
ggsave("EAPC in ASDR.JPEG",width = 12, height = 7,dpi = 1000) #使用ggsave()函数保存，图片质量好
...
# 读取图片  
fig2<- image_read("ASDR总.JPEG")%>% image_ggplot()  

fig1<- image_read("ASMR总.JPEG")%>% image_ggplot()  

fig4<- image_read("EAPC in ASDR.JPEG")%>% image_ggplot()  

fig3<- image_read("EAPC in ASMR.JPEG")%>% image_ggplot()  
print(fig4)
...
ggsave("组合图片.JPEG",width = 12, height = 7,dpi = 1000) #使用ggsave()函数保存，图片质量好
